...
export default async function CaloriePage({params}: Props) {
  const {locale} = await params;
  const t = await getTranslations({locale, namespace: 'calorie'});
  ...
  const canonicalUrl = locale === 'tr'
    ? `${baseUrl}/tr/saglik/kalori-hesaplayici`
    : `${baseUrl}/en/health/calorie-calculator`;
      ...
      <div className="max-w-2xl mx-auto px-4 pb-28 md:pb-12">
        <section className="prose prose-slate max-w-none text-[#0F172A]">
          ...
          <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4 font-mono text-sm text-[#0F172A] space-y-1">
            <p>{t('formulaBmrMale')}</p>
            <p>{t('formulaBmrFemale')}</p>
            <p>{t('formulaBmi')}</p>
          </div>
