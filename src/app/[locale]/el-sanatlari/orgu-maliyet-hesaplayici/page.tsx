...
import KnittingCalculator from '@/components/calculator/KnittingCalculator';
...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({locale, namespace: 'knitting'});
  return {
    title: t('metaTitle'),
...
export default async function KnittingPage({params}: Props) {
  const {locale} = await params;
  const t = await getTranslations({locale, namespace: 'knitting'});
  ...
  const canonicalUrl = locale === 'tr'
    ? `${baseUrl}/tr/el-sanatlari/orgu-maliyet-hesaplayici`
    : `${baseUrl}/en/crafts/knitting-cost-calculator`;
